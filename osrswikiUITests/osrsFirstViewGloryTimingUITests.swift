...
final class osrsFirstViewGloryTimingUITests: XCTestCase {
    func testGloryFirstViewTimingProtocol() throws {
        ...
        try openGlory(disablePrefetch: true, dwellSeconds: 0)
        try openGlory(disablePrefetch: false, dwellSeconds: 0)
        try openGlory(disablePrefetch: false, dwellSeconds: 12)
    }

    private func openGlory(disablePrefetch: Bool, dwellSeconds: TimeInterval) throws {
        let app = XCUIApplication()
        app.terminate()
        app.launchArguments = disablePrefetch ? ["-osrsDisableFirstViewPrewarm"] : []
        app.launch()
        ...
        let result = app.staticTexts.matching(NSPredicate(format: "label == %@", "Amulet of glory")).firstMatch
