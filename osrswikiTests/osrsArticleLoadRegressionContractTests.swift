...
final class osrsArticleLoadRegressionContractTests: XCTestCase {
    func testFirstPaintPrewarmAndSwitcherPinWorkRemainIntact() throws {
        ...
        XCTAssertTrue(articleView.contains("settleArticleScrollAfterRefresh()"))
        XCTAssertTrue(articleWebView.contains("osrsArticleRefreshSettlement.settle"))
        XCTAssertTrue(articleWebView.contains("osrsArticleRefreshSettlement.configure"))
        XCTAssertTrue(viewModel.contains("osrsArticleRefreshSettlement.settle"))
        XCTAssertTrue(viewModel.contains("settleArticleScrollAfterRefresh()"))
    ...
    func testArticleAudioPlayerPrefersMpegAndIsWiredIntoHtmlBuilder() throws {
        ...
        XCTAssertTrue(audioJs.contains("osrs-article-audio-chrome"))
        XCTAssertTrue(audioJs.contains("osrs-article-audio-time"))
        XCTAssertTrue(audioJs.contains("osrs-article-audio-seek"))
        XCTAssertTrue(audioJs.contains("--osrs-article-audio-height"))
        XCTAssertTrue(audioJs.contains("--osrs-article-audio-radius"))
        XCTAssertTrue(audioJs.contains("border:0"))
        XCTAssertFalse(audioJs.contains("min-height:44px"))
        XCTAssertFalse(audioJs.contains("--body-border"))
        XCTAssertFalse(audioJs.contains("wrap.insertBefore(playBtn, audio)"))
        XCTAssertNil(audioJs.range(of: #"(?m)^\s*module\.exports"#, options: .regularExpression))
        let iosCss = try source(root, "platforms/ios/osrswiki/Assets/styles/ios-article-aesthetics.css")
        XCTAssertTrue(iosCss.contains(".osrs-article-audio-chrome"))
        XCTAssertTrue(iosCss.contains("--osrs-article-audio-height"))
        XCTAssertTrue(iosCss.contains("--osrs-article-audio-radius"))
        XCTAssertTrue(iosCss.contains("border: 0"))
        XCTAssertFalse(iosCss.contains("min-height: 44px"))
        let themes = try source(root, "shared/css/themes.css")
        XCTAssertEqual(themes, try source(root, "platforms/ios/osrswiki/Assets/styles/themes.css"))
        XCTAssertTrue(themes.contains("--osrs-article-audio-height: 32px"))
        XCTAssertTrue(themes.contains("--osrs-article-audio-fill: var(--body-main)"))
    }
